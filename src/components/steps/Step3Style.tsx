...
export default function Step3Style() {
  ...
  const [customDialogs, setCustomDialogs]       = useState<CustomDialog[]>(
    project?.customDialogs || [{ id: "d1", speaker: "", text: "" }]
  );
  ...
  return (
    ...
    >
            ...
            <div className="grid grid-cols-3 gap-3">
              {COMIC_STYLES.map((cs) => (
                <button
                  ...
                  className={`p-3 rounded-2xl border-2 text-center transition-all space-y-1 ${
                    comicStyle === cs.id
                      ? "border-brand-400 bg-brand-50 shadow-md"
                      : "border-gray-100 bg-white hover:border-brand-200"
                  }`}
      ...
      <div className="space-y-3">
        ...
        <p className="text-xs text-gray-400">
          {dialogMode === "auto"
            ...
            : "Gib eigene Dialoge ein – die KI ergänzt fehlende automatisch."}
        </p>
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      className="w-32 p-2 rounded-xl border-2 border-brand-100 focus:border-brand-400 focus:outline-none text-sm text-gray-700 bg-white"
                    />
                    {customDialogs.length > 1 && (
                      <button
                        onClick={() => removeDialog(d.id)}
                        className="ml-auto text-gray-300 hover:text-red-400 text-lg"
                      >×</button>
                    )}
                  </div>
                  <input
      ...
      <div className="space-y-2">
        ...
        />
        <p className="text-xs text-gray-400">Diese Inhalte fließen direkt in die Dialog-Generierung ein.</p>
      </div>
