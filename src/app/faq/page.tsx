  ...
  return (
    <div className="border-b border-gray-100 last:border-0">
      <button onClick={() => setOpen(!open)} className="w-full text-left py-5 flex items-center justify-between gap-4">
        <span className="font-medium text-[#1f1a2e] text-base">{q}</span>
        <span className={`text-purple-400 text-xl transition-transform duration-200 ${open ? "rotate-45" : ""}`}>+</span>
      </button>
      <AnimatePresence>
        {open && (
          <motion.div
      ...
      <main className="bg-[#fdfaf7]">
        ...
        <section className="max-w-3xl mx-auto px-6 pt-20 pb-16 space-y-10">
          <div className="text-center space-y-3">
            <h1 className="font-display text-4xl font-semibold text-[#1f1a2e]">Häufige Fragen</h1>
            <p className="text-gray-500 text-lg">Alles was du wissen möchtest – auf einen Blick.</p>
          </div>
        ...
        <section className="bg-purple-50 py-20">
          <div className="max-w-2xl mx-auto px-6">
            <div className="bg-white rounded-2xl border border-purple-100 p-8 space-y-6">
              <div className="space-y-2">
                <h2 className="font-display text-2xl font-semibold text-[#1f1a2e]">Kontakt</h2>
                ...
                <form onSubmit={(e) => { e.preventDefault(); setSent(true); }} className="space-y-4">
                  <div className="grid md:grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <label className="text-sm font-medium text-[#1f1a2e]">Name</label>
                      <input value={name} onChange={(e) => setName(e.target.value)} required placeholder="Dein Name"
                        className="w-full p-3 rounded-xl border border-gray-200 focus:border-purple-400 focus:outline-none text-gray-700 bg-white text-sm" />
                    </div>
                    <div className="space-y-1">
                      <label className="text-sm font-medium text-[#1f1a2e]">E-Mail</label>
                      <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required placeholder="deine@email.de"
                        className="w-full p-3 rounded-xl border border-gray-200 focus:border-purple-400 focus:outline-none text-gray-700 bg-white text-sm" />
                    </div>
                  </div>
                  <div className="space-y-1">
                    <label className="text-sm font-medium text-[#1f1a2e]">Nachricht</label>
                    <textarea value={message} onChange={(e) => setMessage(e.target.value)} required rows={5}
                      placeholder="Deine Frage oder Anmerkung..."
                      className="w-full p-3 rounded-xl border border-gray-200 focus:border-purple-400 focus:outline-none text-gray-700 bg-white resize-none text-sm" />
                  </div>
                  <button type="submit" className="w-full bg-purple-600 text-white py-3 rounded-xl font-medium hover:bg-purple-700 transition-colors">
                    Nachricht senden
                  </button>
                </form>
              )}
